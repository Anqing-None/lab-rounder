...
var Rounder = /** @class */ (function () {
    ...
    Rounder.roundDown = function (num, decimalPlace) {
        ...
        var ret = "".concat(num);
        var roundDownRet = Math.floor(Number("".concat(num, "e").concat(decimalPlace))) / Math.pow(10, decimalPlace);
    ...
    // Absolute Devitation
    Rounder.calcAD = function (numList, refPos) {
        if (refPos === void 0) { refPos = 0; }
    ...
    Rounder.calc2RD = function () {
        ...
        var RD = step1.div(step2).mul(100).toString(); // (a - b) / (a + b)
        return RD;
    };
    Rounder.calcSD = function () {
        var numList = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            numList[_i] = arguments[_i];
        }
        var _numList = numList.map(function (num) { return Number(num); });
        var avg = Rounder.calcAvg.apply(Rounder, _numList);
        var xiMinusAvgPowSum = new Decimal(0); // (xi - X-)2
        for (var i = 0; i < numList.length; i++) {
            var x = numList[i];
    ...
    Rounder.calcRSD = function () {
        ...
        var avg = Rounder.calcAvg.apply(Rounder, _numList);
        // console.log(`avg : ${avg}`)
        var SD = Rounder.calcSD.apply(Rounder, numList);
        // console.log(`SD : ${SD}`)
        // avg 与 SD 经过两次取值是否会有偏差？
        var RSD = new Decimal(SD).div(avg).mul(100).toString();
        // console.log(`RSD : ${RSD}`)
        return RSD;
    };
    Rounder.scientificStrToNum = function (sciStr) {
        ...
        if (sciStr.includes("×10")) {
            ...
            var num = Number(sciStr);
            if (!Number.isFinite(num)) {
                throw new Error("[LabRounder] scientificStrToNum Error with ".concat(sciStr));
            }
            return num;
        }
    };
